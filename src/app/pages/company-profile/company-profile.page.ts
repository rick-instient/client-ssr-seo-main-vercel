...
export class CompanyProfilePage implements OnInit {
  ...
  ionViewDidEnter() {
    ...
    let isCompanyWebsiteUrl = this.route.snapshot.paramMap.get(
      'isCompanyWebsiteUrl'
    );

    if (isCompanyWebsiteUrl) {
      this.config
        .storageSave('isCompanyWebsiteUrl', isCompanyWebsiteUrl)
        .catch();
    }

    if (this.document && this.document.referrer) {
      this.config.storageSave('ReferredBy', this.document.referrer);
    }
  }
