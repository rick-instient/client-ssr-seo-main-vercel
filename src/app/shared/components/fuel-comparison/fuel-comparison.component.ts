...
export class FuelComparisonComponent implements OnChanges {
  ...
  constructor(private cdref: ChangeDetectorRef,     @Inject(DOCUMENT) private document: Document,@Inject(PLATFORM_ID) private platformId) {
  ...
  ngAfterViewInit() {
    ...
    if (this.fuelUserData.length && this.isBrowser) {
      this.createPieChart(this.fuelUserData);
    }
    if (this.heatingOilHistory  && this.isBrowser) {                                                                        
